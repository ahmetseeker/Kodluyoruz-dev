...
const categories = menu.reduce((val, item) => {
...
const categoryList = () => {
  ...
  const categoryBtn = categories.map((category) => {
    return `<button class="btn btn-primary btn-item" data-id="${category}">${category}</button>`;
  }).join("");
...
const menuList = (menu) => {
  let output = '';
  menu.forEach((item) => {
    output += `<div class = "menu-items col-lg-6 col-sm-12">
          <img src=${item.img}
               alt=${item.title}
               class="photo"
          />
